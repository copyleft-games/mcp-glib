...
** McpMuxTransport
I/O-free multiplexed transport. Outbound frames go through a host callback; inbound frames are pushed in by the host. Used to tunnel MCP inside another protocol (e.g. a chat-bridge WebSocket). See the [[file:transport-guide.org#multiplexed-mux-transport][Transport Guide]] for the full embedding pattern.
...
The previous callback's destroy notify runs on replacement and on transport finalize.
...
Frames pushed while the transport is =DISCONNECTED= are silently dropped.
